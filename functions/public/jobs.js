...
exports.sendConfirmation = async (req, res) => {
  ...
  const docId = await performInsert(req.body, 'confirmation');
...
exports.sendApply = async (req, res) => {
  ...
  const docId = await performInsert(req.body, 'applications');
  console.info(`Added document with id ${docId}`);
  return res.status(201).json('');
};
...
async function performInsert(data, collectionPath) {
